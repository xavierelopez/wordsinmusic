...
module.exports = (function() {
	return {	
			...
			if(options.site === 'lyrdb.com') {
				...
				handler.dataParser = function(res, callback) {
					...
					if(res === '') { 
						...
						var match = res.match(idRe);						
						rest.get('http://webservices.lyrdb.com/getlyr.php?q=' + match).on('complete', function(response) {						
							if(_.isFunction(callback)) {
		...
		extractWords: function(options, callback) {
			...
			wordCounts = _(wordsArray).chain()
						...
						//This regex pattern can be improved
							return word.toLowerCase().replace(/,/g, '').replace(/\./g, '').replace(/\'cause/g, 'because')
								...
								.replace(/[\r\n\t]+/g, ' ').replace(/n\'/g, 'ng').replace(/:/g, '').split(' ');	
						})
						.flatten()
			 			.select(function(word){
							return that.isWordAllowed(word) && word.length > 2;
			 			})
		...
		chooseTopWords: function(options, callback) {
			var initializedOn = new Date();
				topWords =  {'topWords': _(options.wordCounts).chain()
					 		.toArray()
		...
		getSimilarWords: function(options, callback) {
